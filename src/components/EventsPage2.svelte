<script lang="ts">
  ...
  import { fetchEvents, type marketEvent } from '../scripts/fetchEvents2';
  import { format, isValid } from 'date-fns';

  let isLoading = true;
  let events: marketEvent[] = [];

  const calender_api_key = import.meta.env.PUBLIC_CALENDER_API_KEY;
  const calender_id = import.meta.env.PUBLIC_G_CALENDER_ID;

  async function fetchData() {
    try {
      // console.log("fetching data 0");
      events = await fetchEvents(calender_api_key, calender_id);
  ...
  function formatTime(event: marketEvent) {
    ...
    try {
        const time_start = format(event.date_start, "h:mm a");
        const time_end = format(event.date_end, "h:mm a");
